...
public class CompressedModel implements IBakedModel {
	...
	private List<BakedQuad> getChessPiecesQuads(int numberOfPieces) {
		...
		// "builtin/generated" items, which are generated from the 2D texture by
		// adding a thickness in the z direction
		...
		final float SOUTH_FACE_POSITION = 1.0F; // the south face of the cube is
												// at z = 1.0F
		final float NORTH_FACE_POSITION = 0.0F; // the north face of the cube is
												// at z = 0.0F
		...
		for (int i = 0; i < numberOfPieces; ++i) {
			...
			// make a baked quad for each side of the chessboard i.e. front and
			// back (south and north)
			final float DELTA_FOR_OVERLAP = 0.001F; // add a small overlap to
													// stop the quad from lying
			...
			BakedQuad nextPieceFront = createBakedQuadForFace(columnCentrePosition, PIECE_WIDTH, rowCentrePosition,
					PIECE_HEIGHT, -DISTANCE_BEHIND_SOUTH_FACE + DELTA_FOR_OVERLAP, ITEM_RENDER_LAYER0,
					chessPieceTexture, EnumFacing.SOUTH);
			BakedQuad nextPieceBack = createBakedQuadForFace(columnCentrePosition, PIECE_WIDTH, rowCentrePosition,
					PIECE_HEIGHT, -DISTANCE_BEHIND_NORTH_FACE + DELTA_FOR_OVERLAP, ITEM_RENDER_LAYER0,
					chessPieceTexture, EnumFacing.NORTH);
	...
	/**
	 * // Creates a baked quad for the given face. // When you are directly
	 * looking at the face, the quad is centred at [centreLR, centreUD] // The
	 * left<->right "width" of the face is width, the bottom<-->top "height" is
	 * height. // The amount that the quad is displaced towards the viewer i.e.
	 * (perpendicular to the flat face you can see) is forwardDisplacement // -
	 * for example, for an EAST face, a value of 0.00 lies directly on the EAST
	 * face of the cube. a value of 0.01 lies // slightly to the east of the
	 * EAST face (at x=1.01). a value of -0.01 lies slightly to the west of the
	 * EAST face (at x=0.99). // The orientation of the faces is as per the
	 ...
	 * @param normal
	 *            the packed representation of the normal vector, see
	...
	/**
	 * Calculate the normal vector based on four input coordinates assumes that
	 * the quad is coplanar but should produce a 'reasonable' answer even if
	 * not.
	 * 
	 * @return the packed normal, ZZYYXX
	 */
	private int calculatePackedNormal(float x1, float y1, float z1, float x2, float y2, float z2, float x3, float y3,
