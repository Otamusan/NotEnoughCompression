...
public class TileCompressedHopper extends TileEntityHopper implements ITileCompressed, IHopper, ITickable {
	...
	public static void registerFixesHopper(DataFixer fixer) {
		fixer.registerWalker(FixTypes.BLOCK_ENTITY,
				new ItemStackDataLists(TileEntityHopper.class, new String[] { "Items" }));
	}
	...
	public void readFromNBT(NBTTagCompound compound) {
		...
		if (compound.hasKey(Lib.MOD_ID + "_blockcompressed")) {
			this.compressedblock = new ItemStack(compound.getCompoundTag(Lib.MOD_ID + "_blockcompressed"));
		}
	...
	public NBTTagCompound writeToNBT(NBTTagCompound compound) {
		...
		if (compressedblock != null) {
			...
			compound.setTag(Lib.MOD_ID + "_blockcompressed", itemnbt);
		}
	...
	public static Pair<IItemHandler, Object> getItemHandler(World worldIn, double x, double y, double z,
		...
		if (block.hasTileEntity(state)) {
			...
			if (tileentity != null) {
				if (tileentity.hasCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, side)) {
					IItemHandler capability = tileentity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY,
							side);
				...
				if (inventoryWasEmpty && destination instanceof TileEntityHopper) {
					...
					if (!destinationHopper.mayTransfer()) {
						...
						if (source instanceof TileEntityHopper) {
							if (destinationHopper.getLastUpdateTime() >= ((TileEntityHopper) source)
									.getLastUpdateTime()) {
								k = 1;
							}
						}
	...
	public static boolean insertHook(TileEntityHopper hopper) {
		...
		if (destinationResult == null) {
			...
			if (isFull(itemHandler)) {
				...
				long sum = sum((TileCompressedHopper) hopper);
				for (int i = 0; i < hopper.getSizeInventory(); ++i) {
					if (!hopper.getStackInSlot(i).isEmpty()) {
						ItemStack originalSlotContents = hopper.getStackInSlot(i).copy();
						int count = originalSlotContents.getCount();
						ItemStack remainder;
						if (sum > count) {
							sum -= count;

							remainder = putStackInInventoryAllSlots(hopper, destination, itemHandler,
									hopper.decrStackSize(i, count));
						} else {

							remainder = putStackInInventoryAllSlots(hopper, destination, itemHandler,
									hopper.decrStackSize(i, (int) sum));
						}
			...
			if (this.isInventoryFull(iinventory, enumfacing)) {
				...
				for (int i = 0; i < this.getSizeInventory(); ++i) {
					if (!this.getStackInSlot(i).isEmpty()) {
						...
						if (sum > count) {
							sum -= count;
							itemstack1 = putStackInInventoryAllSlots(this, iinventory, this.decrStackSize(i, count),
									enumfacing);
						} else {
							itemstack1 = putStackInInventoryAllSlots(this, iinventory, this.decrStackSize(i, (int) sum),
									enumfacing);
						}
	...
	private long sum() {
		return (long) Math.pow(8, ItemCompressed.getTime(getItemCompressed()) - 1);
	}

	private static long sum(TileCompressedHopper tileCompressedHopper) {
		return (long) Math.pow(8, ItemCompressed.getTime(tileCompressedHopper.getItemCompressed()) - 1);
	}
		...
		if (canInsertItemInSlot(destination, stack, index, direction)) {
			...
			if (flag) {
				if (flag1 && destination instanceof TileEntityHopper) {
					TileCompressedHopper tileentityhopper1 = (TileCompressedHopper) destination;

					if (!tileentityhopper1.mayTransfer()) {
						int k = 0;

						if (source != null && source instanceof TileEntityHopper) {
							TileCompressedHopper tileentityhopper = (TileCompressedHopper) source;
	...
	private IInventory getInventoryForHopperTransfer() {
		EnumFacing enumfacing = BlockHopper.getFacing(Blocks.HOPPER.getMetaFromState(getState()));
