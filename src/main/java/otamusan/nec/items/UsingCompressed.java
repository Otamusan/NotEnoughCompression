...
public class UsingCompressed {
    public static ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand) {
        ...
        if (ItemCompressed.getTime(compressed) > NECConfig.CONFIG_TYPES.using.usingCompressionTime)
            return new ActionResult<ItemStack>(EnumActionResult.PASS, compressed);
        ...
        for (int i = 0; i < n; i++) {
            ...
            ActionResult<ItemStack> result = use.useItemRightClick(world, player, hand);
            results.add(use.useItemRightClick(world, player, hand));

            ItemStack remain = player.getHeldItem(hand);
            if (remain.isEmpty()) {
                remains.addItem(result.getResult().copy());
            }else{
